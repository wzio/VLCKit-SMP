...
ditto -c -k --sequesterRsrc --keepParent "$IOS_LOCATION" ".tmp/MobileVLCKit.xcframework.zip"

#Update package file
PACKAGE_HASH=$(sha256sum ".tmp/MobileVLCKit.xcframework.zip" | awk '{ print $1 }')
PACKAGE_STRING="Target.binaryTarget(name: \"MobileVLCKit\", url: \"https:\/\/github.com\/wzio\/VLCKit-SMP\/releases\/download\/$TAG_VERSION\/MobileVLCKit.xcframework.zip\", checksum: \"$PACKAGE_HASH\")"
